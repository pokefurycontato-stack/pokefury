window.copyPlayerSpawns = async function() {
    const { data: regions, error: e1 } = await window.supabase.from('regions').select('id, name');
    if (e1) throw e1;

    const kantoRegion = regions.find(r => r.name === 'Kanto');
    const { data: kanto, error: e2 } = await window.supabase.from('region_maps').select('id, name, player_spawn_x, player_spawn_y').eq('region_id', kantoRegion.id);
    if (e2) throw e2;
    ...
    for (const region of regions) {
        ...
        const { data: maps } = await window.supabase.from('region_maps').select('id, name').eq('region_id', region.id);
        for (const map of maps) {
            const source = kantoMap[map.name];
            if (source && source.player_spawn_x != null && source.player_spawn_y != null) {
                const { error } = await window.supabase.from('region_maps').update({ player_spawn_x: source.player_spawn_x, player_spawn_y: source.player_spawn_y }).eq('id', map.id);
