...
export class AFKManager {
    ...
    async _healTeamAfterBattle() {
        ...
        if (cityPartyList) game.updatePartyPanel(cityPartyList);
    ...
    async _handleCityBattleZone() {
        ...
        if (!this._cityFoughtPositions) this._cityFoughtPositions = [];

        const activePokemon = city.wildPokemon.filter(p => p.active);
        if (activePokemon.length === 0) {
            this._cityPath = null;
            return;
        }

        const lastFought = this._cityFoughtPositions[this._cityFoughtPositions.length - 1] || null;

        let nearest = null;
        let minDist = Infinity;
        for (const p of activePokemon) {
            if (lastFought && p.pos_x === lastFought.x && p.pos_y === lastFought.y) continue;
        ...
        if (!nearest) {
            nearest = activePokemon.reduce((best, p) => {
                const d = Math.hypot(p.pos_x - city.playerX, p.pos_y - city.playerY);
                return d < best.d ? { p, d } : best;
            }, { p: null, d: Infinity }).p;
            if (!nearest) return;
        }

        const targetKey = nearest.pos_x + ',' + nearest.pos_y;
        if (this._cityTargetId !== targetKey) {
            this._cityPath = null;
            this._cityTargetId = targetKey;
        }
        ...
        const path = city.findPath(city.playerX, city.playerY, nearest.pos_x, nearest.pos_y);
    ...
    _recordCityFight(pos_x, pos_y) {
        if (!this._cityFoughtPositions) this._cityFoughtPositions = [];
        this._cityFoughtPositions.push({ x: pos_x, y: pos_y });
        if (this._cityFoughtPositions.length > 10) {
            this._cityFoughtPositions.shift();
        }
    }
