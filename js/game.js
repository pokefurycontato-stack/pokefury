...
import { AFKManager } from './afk.js?v=20260820e';
...
import { InfiniteTowerManager } from './infinite-tower.js?v=20260820b';
...
class PokeFuryGame {
    ...
    async startGame(starterSpecies) {
        ...
        if (!window.adminOverworldAccess) {
            ...
            const cs = document.getElementById('city-screen');
        ...
        if (!window.adminOverworldAccess) {
            ...
            if (window.cityScreen && typeof window.cityScreen.open === 'function') {
                window.cityScreen.open();
            }
        }
    }

    applyNonAdminCityMode() {
        // Esconde o acesso ao overworld/mapa para jogadores nao-admin
        document.querySelectorAll('.nav-btn[data-screen="map"]').forEach(btn => { btn.style.display = 'none'; });
        const minimap = document.getElementById('right-minimap');
        if (minimap) minimap.style.display = 'none';
        // Remove o botao de fechar a cidade (X) para nao-admin
...
if (this._professorOriginalOrder) {
    ...
    renderBattle() {
        ...
        let activeEnemy;
        if (this._enemySwitching) {
            activeEnemy = this._lastBattleEnemy;
        } else if ((this._isTowerBattle || this._isGymBattle) && this._lastBattleEnemy && this.enemyTeam.includes(this._lastBattleEnemy)) {
            activeEnemy = this._lastBattleEnemy;
        } else {
            activeEnemy = getFirstAlive(this.enemyTeam) || this._lastBattleEnemy;
        }
        ...
        return new Promise(async (resolve) => {
            ...
            if (teamList) {
                ...
                teamList.addEventListener('drop', (e) => {
                    ...
                    teamList.innerHTML = renderTeamList();
                });
            }
        });
    }
    ...
    async startBattleWithPokemon(pokemonName, level, spriteUrl, isShinyOverride, opts = {}) {
        ...
        try {
            ...
            let introMsg;
            if (gymLeader) {
                introMsg = `${pokemon.name} do Líder de Ginásio ${gymLeader.name} apareceu, prepare-se para a luta!`;
            } else {
    ...
    async endBattle(result) {
        ...
        if (result) {
            ...
            if (result === 'win') {
                window.ProfessorQuests?.incrementProfessorQuestProgress?.('battles_total', 1);
                window.ProfessorQuests?.incrementProfessorQuestProgress?.('battles_biome', 1, this._currentBiome || null);
            }
        }
    ...
    updatePartyPanel(listEl, opts = {}) {
        ...
        if (opts.withHeal !== false) {
            const btnId = list.id === 'city-party-list' ? 'city-heal-pokemon-btn' : 'heal-pokemon-btn';
    ...
    setupAdminButtons() {
        ...
        if (profileAdminBtn) {
            profileAdminBtn.onclick = () => {
                if (window.profileAdmin) window.profileAdmin.open();
            };
        }
        ...
        const skinShopBtn = document.getElementById('btn-skin-shop');
        if (skinShopBtn) {
            skinShopBtn.onclick = () => {
                if (window.premiumStore) {
                    window.premiumStore.setCurrentChar(this.currentCharacterId);
...
openEventsPanel() {
    ...
    setupDonateItemSearch() {
        ...
        document.getElementById('donate-item-btn').onclick = async () => {
            ...
            const { error } = await window.db.rpc('safe_add_item', { p_character_id: this._donateSelectedChar.id, p_item_id: itemId, p_quantity: qty });
    ...
    setupDonatePokemonForm() {
        ...
        document.getElementById('donate-poke-btn').onclick = async () => {
            ...
            const { data: stored, error: pcError } = await window.db.rpc('safe_store_pc_pokemon', {
                p_character_id: this._donateSelectedChar.id,
                p_pokemon_id: pokemonData.id,
                p_species: pokemonData.species,
                p_nickname: pokemonData.name,
                p_level: level,
                p_current_hp: stats.hp,
                p_max_hp: stats.hp,
                p_experience: Math.floor(Math.pow(level, 3) * 0.8),
                p_moves: moveIds.map(id => ({ id: String(id), pp: 35 })),
                p_iv_hp: ivs.hp || 0, p_iv_attack: ivs.attack || 0, p_iv_defense: ivs.defense || 0,
                p_iv_sp_atk: ivs.spAtk || 0, p_iv_sp_def: ivs.spDef || 0, p_iv_speed: ivs.speed || 0,
                p_ev_hp: evs.hp || 0, p_ev_attack: evs.attack || 0, p_ev_defense: evs.defense || 0,
                p_ev_sp_atk: evs.spAtk || 0, p_ev_sp_def: evs.spDef || 0, p_ev_speed: evs.speed || 0,
                p_nature: nature,
                p_happiness: 70,
                p_is_shiny: false
            });
            if (pcError || !stored?.success) {
                donateError = { message: pcError?.message || 'Erro ao salvar no PC' };
            } else {
                donatedTo = 'PC Pokemon (Box ' + (stored.box || '?') + ')';
            }
    ...
    async startPVPBattle(challenge) {
        ...
        if (LS) { LS.setProgress(100); setTimeout(() => LS.hide(), 300); }
    }
